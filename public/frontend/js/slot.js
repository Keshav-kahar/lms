let bankValue = document.currentScript.getAttribute('data-balance');
let games = document.currentScript.getAttribute('data-games');
let route = document.currentScript.getAttribute('data-route');
...
const Slot = function (canvas) {
  ...
  this.checkout = function () {
    if (confirm("Are you sure? We can keep your money better!")) {
      ...
      let toast = {
        title: 'Opps',
        message: "Your operation has been made successfully",
  ...
  this.spin = function () {
    ...
    // console.log(conf.bet.value * 1);

    var item = {
        beting_amount: conf.bet.value * 1,
        win_bonus: null,
        pay_out: conf.bet.value * 1,
        type: "LOSE"
    }
    postdata(item);
  };
  ...
  this.start = function () {
    conf.sound.win.volume = 1;
    conf.sound.spin.volume = 1;
    reels.forEach((reel) => {
      reel.clicked = true;
      reel.finalAnimation = false;
      reel.finalShapes = reel.setFinalShapes();
...
let conf = {
  ...
  autoModeDelay: 3000,
  sound: {
    win: new Audio(baseurl+"/images/games/audio/slot_win.mp3"),
    spin: new Audio(baseurl+"/images/games/audio/spin.mp3")
  }
};
//Resource loader
Resources(
  baseurl+"/images/games/slots/BAR.png",
  baseurl+"/images/games/slots/2xBAR.png",
  baseurl+"/images/games/slots/3xBAR.png",
  baseurl+"/images/games/slots/7.png",
  baseurl+"/images/games/slots/Cherry.png"
  ...
  names.forEach(function (name) {
    let key = name.replace(
        new RegExp(
          "^("+baseurl+"/images/games/slots/)|(.png|.jpg|.jpeg)$",
          "ig"
        ),
        ""
      ),
