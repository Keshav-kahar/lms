...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['games'] = Game::where('status', 1)->latest()->get();
        ...
        if (!is_null($request->type)) {
            switch ($request->type) {
                ...
                case "Win Game":
                    ...
                    break;
                case "Lose Game":
                    $trans->where('status', 11);
                    break;
                default:
                    $trans->whereIn('status', [1, 2, 3, 4, 5, 10, 11]);
            }
        }
    ...
    {
        $data['page_title'] = "Game List";
        $data['games'] = Game::where('status', 1)->latest()->get();
        return view('user.game.index', $data);
    }

    public function gameDetails($slug)
    {
        $games = Game::where('status', 1)->whereSlug($slug)->first();
        $data['page_title'] = $games->name;
        if ($slug) {
            if ($slug == 'roulette') {
                return view('user.game.roulette_wheel', $data, compact('games'));
            } elseif ($slug == 'slots') {
                return view('user.game.slot', $data, compact('games'));
            } else {
                dd($slug);
            }
        }
        return redirect()->route('games-list')->with('alert', 'Not found.');
    }

    public function rouletteStore(Request $request)
    {
        $me = User::find(Auth::id());
        try {
            $games = Game::whereSlug('roulette')->first();
            if ($me->balance >= $request->beting_amount) {
                if ($request->type == "WIN") {
                    $new_balance = $me->balance + $request->pay_out;
                    $me->balance = $new_balance;
                    $me->update();
                    createTransaction("Winning Bonus via " . $games->name, $request->pay_out, $me->balance, $new_balance, 10, Null, '+');
                    return response()->json([
                        'success' => true,
                        'message' => __('Congratulations, You won.')
                    ]);
                } elseif ($request->type == "LOSE") {
                    $new_balance = $me->balance - $request->beting_amount;
                    $me->balance = $new_balance;
                    $me->update();
                    createTransaction("Lose Bonus via " . $games->name, $request->beting_amount, $me->balance, $new_balance, 11, Null, '-');
                    return response()->json([
                        'success' => false,
                        'message' => __('You lose. Pleaes try again.')
                    ]);
                }
                return response()->json([
                    'success' => false,
    ...
    public function slotStore(Request $request)
    {
        $me = User::find(Auth::id());
        try {
            $games = Game::whereSlug('slots')->first();
            if ($me->balance >= $request->beting_amount) {
                if ($request->type == "WIN") {
                    $new_balance = $me->balance + $request->pay_out;
                    $me->balance = $new_balance;
